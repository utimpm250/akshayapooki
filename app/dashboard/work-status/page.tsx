...
export default function WorkStatusPage() {
  ...
  const handleStatusChange = (id: string, newStatus: string) => {
    const workToUpdate = works.find(w => (w.id || w.reference) === id);
    if (workToUpdate) {
      const updated = { ...workToUpdate, status: newStatus };
      updateWork(updated);
      setWorks(getWorks());
    }
  };
  ...
  const handleDelete = (id: string) => {
    if (confirm("തീർച്ചയായും ഈ വർക്ക് ഡിലീറ്റ് ചെയ്യണമെന്നുണ്ടോ?")) {
      deleteWork(id);
      setWorks(getWorks());
    }
  };
  ...
  const counts = {
    ...
    completed: works.filter(w => w.status === "Completed" || w.status === "Delivered").length,
    returned: works.filter(w => w.status === "Returned" || w.status === "Rejected").length,
  };
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
          ...
          <div className="bg-red-50 border border-red-200 rounded-2xl p-5">
            <p className="text-sm text-red-700 font-medium">Returned / Rejected</p>
            <h2 className="text-3xl font-bold mt-2">{counts.returned}</h2>
          </div>
        </div>
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col items-start lg:items-end gap-3">
                  ...
                  <select
                    value={work.status}
                    onChange={(e) => handleStatusChange(workId, e.target.value)}
